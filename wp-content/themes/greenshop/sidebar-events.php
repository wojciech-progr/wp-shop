<div class="col-md-4">
    <div class="popular">
        <h2 class="popular__heading">
            Most popular <br> upcoming events
        </h2>
        <p class="popular__subHeading">
            Check out the most popular events
            and go to unique places and go to unique places that have delighted others
        </p>
        <a href="#" class="popular__article">
            <article>
                <h3 class="popular__title">Event title</h3>
                <img class="popular__image" width="100%" height="" alt=""
                    src="<?= GREENSHOP_THEME_URL ?>/images/bricks.jpeg">
                <p class="popular__description">
                    Description of event Lorem Ipsum Description of event Lorem Ipsum
                    Description of event Lorem Ipsum Description of event Lorem Ipsum
                </p>
                <span class="popular__counter">1333 views this month</span>
            </article>
        </a href="">
        <a href="#" class="popular__article">
            <article>
                <h3 class="popular__title">Event title</h3>
                <img class="popular__image" width="100%" height="" alt=""
                    src="<?= GREENSHOP_THEME_URL ?>/images/bricks.jpeg">
                <p class="popular__description">
                    Description of event Lorem Ipsum Description of event Lorem Ipsum
                    Description of event Lorem Ipsum Description of event Lorem Ipsum
                </p>
                <span class="popular__counter">1333 views this month</span>
            </article>
        </a href="">
        <a href="#" class="popular__article">
            <article>
                <h3 class="popular__title">Event title</h3>
                <img class="popular__image" width="100%" height="" alt=""
                    src="<?= GREENSHOP_THEME_URL ?>/images/bricks.jpeg">
                <p class="popular__description">
                    Description of event Lorem Ipsum Description of event Lorem Ipsum
                    Description of event Lorem Ipsum Description of event Lorem Ipsum
                </p>
                <span class="popular__counter">1333 views this month</span>
            </article>
        </a href="">
    </div>
</div>